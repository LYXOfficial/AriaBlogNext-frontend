...
`;
  ...
  static foldingTag(markdown: string) {
    const foldingRegex =
      /{%\s*folding\s+(\w+)(?:\s+open)?\s*,\s*([^%]+)\s*%}([\s\S]*?){%\s*endfolding\s*%}/g;
    return markdown.replace(
      foldingRegex,
      (match, color: string, summary, content) => {
...
`;
  ...
  static hideToggleTag(markdown: string) {
    const foldingRegex =
      /{%\s*hideToggle(?:\s*(open)\s*,)?\s*([^%]+?)\s*%}([\s\S]*?){%\s*endhideToggle\s*%}/g;
...
<span>${text}</span>
  ...
  static noteTag(markdown: string) {
    const noteRegex =
      /{%\s*note\s+(\w+)(?:\s+\w+)?\s*%}([\s\S]*?){%\s*endnote\s*%}/g;
...
`;
  ...
  static tipTag(markdown: string) {
    const noteRegex =
      /{%\s*tip\s+(\w+)(?:\s+\w+)?\s*%}([\s\S]*?){%\s*endtip\s*%}/g;
    return markdown.replace(noteRegex, (match, icon, content) => {
      icon = icon.toLowerCase().trim();
      content = content.trim();
...
`;
  ...
  static tabsTag(markdown: string) {
    ...
    return markdown.replace(tabsRegex, (match, tabsContent) => {
      ...
      tabsContent.replace(
        tabContentRegex,
        (match: string, tabTitle: string, tabBody: string) => {
...
`;
  ...
  static chatTag(markdown: string) {
    const chatRegex =
      /{%\s*chat\s+(left|right)\s*,\s*([^%]+)\s*%}([\s\S]*?)\s*{%\s*endchat\s*%}/g;
...
export class MarkdownRewriter {
  ...
  runRewrite() {
    return this.filter.reduce(
      (markdown, filter) => filter(markdown),
      this.markdown,
    );
  }
}
